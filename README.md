...
HydraBit 是一个极简、像素风的桌面小挂件，用来追踪饮水和补剂微习惯。它常驻在桌面角落，通过键盘活跃度判断水蓝蓝是否“口渴”，不弹窗、不发系统通知，只用桌宠状态和快捷键记录来形成轻量的饮水习惯。
...
HydraBit 不需要登录，也不使用云同步。设置、今日数据和历史饮水数据会保存在 Electron 的本地 `userData` 目录中，并会按日期自动重置今日统计。
